...
본 테스트는 scope=app entry "kakao"에 alias "링키지접근성"을 달고
...
def test_alias_hit_on_app_scope_entry_plays_single_note(plugin, monkeypatch):
    ...
    store.save(plugin.appListFile, ["kakao"], scopes={"kakao": SCOPE_APP})
    assert store.set_aliases(plugin.appListFile, "kakao", ["링키지접근성"])
    ...
    plugin._match_and_beep("", "링키지접근성", tab_sig=0x2001)
...
def test_alias_registered_in_window_lookup_for_app_scope(plugin):
    ...
    store.save(plugin.appListFile, ["kakao"], scopes={"kakao": SCOPE_APP})
    store.set_aliases(plugin.appListFile, "kakao", ["링키지접근성"])
    plugin.appList = store.load(plugin.appListFile)
    plugin._rebuild_lookup()
