...
에서만 매칭 위임이 발생해야 한다. 자식 요소의 nameChange(obj.name != foreground.name)는
스킵되어야 한다.
"""
...
def test_obj_name_matches_foreground_name_triggers_match(captured_match, mock_api, debug_off):
    """최상위 창 자체의 name 변경 (Ctrl+Tab 탭 확정 전환)은 매칭 위임."""
    plugin, calls = captured_match
    foreground = _make_obj(name="main.cpp - Notepad++", hwnd=0xC100, appName="notepad++")
    mock_api(foreground)
    ...
    assert tab_signature == 0xC100


def test_obj_name_differs_from_foreground_name_is_skipped(captured_match, mock_api, debug_off):
    """웹 DOM / 동적 레이블 등 자식 객체의 nameChange는 foreground.name과 불일치 → skip."""
    plugin, calls = captured_match
    foreground = _make_obj(name="Firefox - example.com", hwnd=0xF100, appName="firefox")
...
def test_empty_foreground_name_is_skipped(captured_match, mock_api, debug_off):
    """foreground.name이 비어있으면 skip (조기 컷 통과 후 foreground 제목 없음 단계).

    조기 컷은 hwnd 비교이므로 obj/fg가 같은 창(hwnd 동일)이라도 통과하지만,
    그 이후 foreground.name이 empty면 매칭 위임 전에 skip. obj.name 자체는 더 이상
    읽지 않음 (foreground.name만이 탭 제목의 진실된 소스).
    """
    plugin, calls = captured_match
    foreground = _make_obj(name="", hwnd=0xA100, appName="chrome")
    mock_api(foreground)
...
def test_tab_signature_uses_obj_hwnd(captured_match, mock_api, debug_off):
    """tab_signature는 obj.windowHandle 값. Ctrl+Tab 케이스에선 obj==fg이므로 둘 다 같은 값."""
    plugin, calls = captured_match
    foreground = _make_obj(name="YouTube - Firefox", hwnd=0x7777, appName="firefox")
    mock_api(foreground)
...
def test_invalid_window_handle_is_cut(captured_match, mock_api, debug_off):
    ...
    이전 구현은 tab_signature=0 폴백으로 매칭까지 진행했지만, hwnd 비교 기반 조기
    컷에서는 판정 불가 → 보수적으로 return. 실환경에서 NVDAObject의
    windowHandle이 예외를 던지는 건 극히 드물며, 그 상태로 매칭을 진행해봐야
    tab_signature=0이 되어 signature_guard가 무의미해진다.
    """
    plugin, calls = captured_match
    foreground = MagicMock()
    foreground.name = "Some Title - App"
    foreground.appModule = MagicMock()
    foreground.appModule.appName = "someapp"
    # windowHandle 접근 시 예외를 던지는 케이스 모사 (property 예외)
    type(foreground).windowHandle = property(lambda self: (_ for _ in ()).throw(RuntimeError("handle gone")))
    mock_api(foreground)
...
def test_menu_item_obj_is_cut_before_name_compare(captured_match, mock_api, debug_off):
    ...
    obj.name과 foreground.name이 우연히 같은 문자열이어도 identity 비교가 먼저라서
    matcher까지 가지 않는다. 실사용에서 메뉴/DOM 자식 이벤트가 여기 걸려
    DBG 로그 노이즈도 생략된다.
    """
    plugin, calls = captured_match
    foreground = _make_obj(name="YouTube - Mozilla Firefox", hwnd=0x1000, appName="firefox")
...
def test_fg_identity_allows_match(captured_match, mock_api, debug_off):
    ...
    plugin, calls = captured_match
    foreground = _make_obj(name="document.txt - Notepad++", hwnd=0xC200, appName="notepad++")
    mock_api(foreground)
