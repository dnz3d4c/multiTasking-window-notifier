# -*- coding: utf-8 -*-
"""eventRouter.dispatch_focus 3분기 라우팅 단위 테스트.

dispatch_focus는 event_gainFocus의 분기 판정 전용. 실제 매칭/비프는
plugin._match_and_beep로 위임되므로, 여기서는 "어떤 title/tab_signature로
위임이 호출되는가"만 검증한다.
"""
...
def _make_obj(window_class_name="", name="", hwnd=0x1000, appName=""):
    """NVDAObject 스텁. windowHandle/appModule.appName까지 포함."""
    obj = MagicMock()
    obj.windowClassName = window_class_name
    obj.name = name
...
def mock_api(monkeypatch):
    """api.getForegroundObject 반환을 테스트별 foreground 객체로 교체 가능한 헬퍼."""
    state = {"foreground": None}

    def _set_fg(foreground):
...
def tab_classes_noop(monkeypatch):
    ...
    monkeypatch.setattr(eventRouter.tabClasses, "is_overlay_class", lambda appId, window_class_name: False)
    monkeypatch.setattr(eventRouter.tabClasses, "is_editor_class", lambda appId, window_class_name: False)
...
def test_dispatch_none_obj_is_noop(captured_match, mock_api, tab_classes_noop, debug_off):
    ...
    eventRouter.dispatch_focus(plugin, None)

    assert calls == []


def test_alt_tab_overlay_uses_obj_name(captured_match, mock_api, tab_classes_noop, debug_off):
    """Alt+Tab 진입은 (obj.window_class_name AND foreground.window_class_name) 두 축 AND. obj.name을 raw title로 사용.

    후보 창의 obj.appId는 오버레이 호스트('explorer')라 무의미하므로 빈 문자열로
    Matcher에 전달되어 app_lookup 조회를 자동 스킵한다.
    """
    plugin, calls = captured_match
    obj = _make_obj(window_class_name=ALT_TAB_OVERLAY_WCN, name="YouTube - Chrome", hwnd=0xAAA, appName="chrome")
    foreground = _make_obj(window_class_name=ALT_TAB_HOST_FG_WCN, name="작업 전환", hwnd=0xBBB, appName="explorer")
    mock_api(foreground)
...
def test_alt_tab_overlay_skipped_when_fg_not_shell_host(
    ...
    """obj.wcn이 InputSite여도 foreground.wcn이 Xaml Shell 호스트가 아니면 Alt+Tab 분기 미진입.

    Win+B 숨김 아이콘·시스템 트레이·알림 센터처럼 같은 UWP InputSite 껍데기를
    쓰는 다른 목록형 UI에서 오탐이 나지 않아야 한다. 본 Phase의 핵심 회귀 방지.
    """
    plugin, calls = captured_match
    # Win+B 시나리오: obj.wcn은 InputSite, foreground.wcn은 시스템 트레이/탐색기 등.
    obj = _make_obj(window_class_name=ALT_TAB_OVERLAY_WCN, name="메신저앱", hwnd=0xD001, appName="explorer")
    foreground = _make_obj(window_class_name="Shell_TrayWnd", name="작업 표시줄", hwnd=0xD000, appName="explorer")
    mock_api(foreground)
...
def test_app_overlay_uses_obj_name(captured_match, mock_api, monkeypatch, debug_off):
    """앱별 overlay (Notepad++ MRU 등): foreground_class_name이 overlay 목록이면 obj.name 사용."""
    plugin, calls = captured_match
    obj = _make_obj(window_class_name="SysListView32", name="main.cpp", hwnd=0xC001, appName="notepad++")
    foreground = _make_obj(window_class_name="#32770", name="MRU", hwnd=0xC000, appName="notepad++")
    mock_api(foreground)

    monkeypatch.setattr(
        eventRouter.tabClasses,
        "is_overlay_class",
...
def test_editor_branch_uses_foreground_name(captured_match, mock_api, monkeypatch, debug_off):
    """editor 자식 컨트롤 분기: foreground.name을 raw title로 쓰고 tab_signature는 obj(자식) hwnd.

    같은 최상위 창이 여러 "제목 없음" 탭을 가질 때 자식 hwnd로 탭 구분이 돼야 함 —
    매칭 회귀 방지의 핵심 불변.
    """
    plugin, calls = captured_match
    obj = _make_obj(window_class_name="RichEditD2DPT", name="", hwnd=0xE001, appName="notepad")
    foreground = _make_obj(window_class_name="Notepad", name="제목 없음 - 메모장", hwnd=0xF001, appName="notepad")
    mock_api(foreground)
...
def test_editor_branch_skipped_when_child_wcn_equals_foreground_class_name(
    ...
    `window_class_name != foreground_class_name` 게이트가 이 폭발 방지용 — 없으면 모든 focus 전환이
    editor 분기로 빠져 과도 매칭.
    """
    plugin, calls = captured_match
    obj = _make_obj(window_class_name="MozillaWindowClass", name="x", hwnd=0xF100, appName="firefox")
    foreground = _make_obj(window_class_name="MozillaWindowClass", name="Some Page - Firefox", hwnd=0xF100, appName="firefox")
    mock_api(foreground)

    monkeypatch.setattr(eventRouter.tabClasses, "is_overlay_class", lambda a, w: False)
    # is_editor_class가 True를 돌려줘도 window_class_name==foreground_class_name이면 분기 진입 불가.
    monkeypatch.setattr(eventRouter.tabClasses, "is_editor_class", lambda a, w: True)
...
def test_no_branch_matches_is_silent(captured_match, mock_api, tab_classes_noop, debug_off):
    ...
    plugin, calls = captured_match
    obj = _make_obj(window_class_name="Button", name="Some Button", hwnd=0xB001, appName="explorer")
    foreground = _make_obj(window_class_name="CabinetWClass", name="탐색기", hwnd=0xB000, appName="explorer")
    mock_api(foreground)
...
def test_empty_raw_title_is_skipped(captured_match, mock_api, monkeypatch, debug_off):
    ...
    plugin, calls = captured_match
    obj = _make_obj(window_class_name=ALT_TAB_OVERLAY_WCN, name="   ", hwnd=0xA001, appName="chrome")
    foreground = _make_obj(window_class_name="Chrome_WidgetWin_1", name="Chrome", hwnd=0xA000, appName="chrome")
    mock_api(foreground)
...
def test_debug_logging_path_is_invoked(captured_match, mock_api, monkeypatch, tab_classes_noop):
    ...
    """
    plugin, calls = captured_match
    monkeypatch.setattr(eventRouter.settings, "get", lambda key: key == "debugLogging")
    obj = _make_obj(window_class_name="Button", name="Some Button", hwnd=0xD001, appName="explorer")
    foreground = _make_obj(window_class_name="CabinetWClass", name="탐색기", hwnd=0xD000, appName="explorer")
    mock_api(foreground)
...
def test_whitespace_only_name_is_skipped(captured_match, mock_api, monkeypatch, debug_off):
    ...
    plugin, calls = captured_match
    obj = _make_obj(window_class_name=ALT_TAB_OVERLAY_WCN, name="\t  \n", hwnd=0xA001, appName="chrome")
    foreground = _make_obj(window_class_name="Chrome_WidgetWin_1", name="Chrome", hwnd=0xA000, appName="chrome")
    mock_api(foreground)
