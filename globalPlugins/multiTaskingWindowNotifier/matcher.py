...
Matcher는 GlobalPlugin 인스턴스를 참조해 다음을 조회한다:
    ...
    - plugin._lookup        (LookupIndex 인스턴스 — windowLookup / appLookup 소유)
    - plugin._notify_switch / _maybe_flush  (디바운스 flush 메서드)
...
class LookupIndex:
    ...
    - windowLookup: scope=window entry + alias를 가진 scope=app entry의 title-only
      역매핑 테이블.
        ...
        4) aliases 필드 — scope=window/app 양쪽 entry의 alias 각각도 setdefault로
           역매핑 추가. foreground title과 Alt+Tab 오버레이 이름이 다른 앱을
           단일 entry로 매칭하기 위함.
