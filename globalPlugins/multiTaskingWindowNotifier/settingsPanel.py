...
class MultiTaskingSettingsPanel(SettingsPanel):
    ...
    def onSave(self):
        ...
        # Phase 5: Humor Pack 옵트인 1회성 경고. optIn=True 프리셋은 설치 직후
        # 사용자가 모르고 선택할 수 있으니 실제 저장 시점에 공공장소 주의 안내를
        # 한 번만 띄운다. config 플래그로 재선택 시엔 조용히 통과.
        #
        # `gui.messageBox`는 NVDA 관용 래퍼로 Win32 MessageBox와 동작 동일.
        # NVDA 코어(gui/settingsDialogs.py의 GeneralSettingsPanel.onSave 등)도
        # onSave 중에 같은 패턴으로 modal을 띄워 검증된 경로.
        new_preset_dict = PRESETS.get(new_preset)
        if (new_preset_dict is not None
                and new_preset_dict.get("optIn")
                and not conf["humorPackWarningShown"]):
            # Translators: Humor Pack 프리셋 첫 선택 시 1회성 안내 다이얼로그 제목.
            title = _("유머 프리셋 안내")
            # Translators: Humor Pack 프리셋 첫 선택 시 1회성 안내 본문.
            body = _(
                "이 프리셋은 방귀/트림 같은 유머성 소리를 포함합니다. "
                "회의나 공공장소에서는 다른 프리셋을 선택하시기를 권장합니다."
            )
            gui.messageBox(body, title, wx.OK | wx.ICON_INFORMATION, self)
            conf["humorPackWarningShown"] = True
