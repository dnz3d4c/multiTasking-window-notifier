...
세 진입점 모두 stateless. dedup 상태는 matcher(Matcher.last_event_signature)가 소유.
...
def handle_foreground(plugin, obj) -> None:
    ...
    # tab_signature: foreground 창의 hwnd. 같은 (appId, title) 조합이라도 다른 hwnd면
    # 다른 시그니처로 처리되어 matcher의 dedup 가드와 자연스럽게 맞물린다.
    try:
        tab_signature = int(getattr(obj, "windowHandle", 0) or 0)
    except Exception:
        tab_signature = 0
    if settings.get("debugLogging"):
        log.info(
            f"mtwn: DBG foreground appId={appId!r} title={title!r} tab_signature={tab_signature}"
        )
...
def handle_name_change(plugin, obj) -> None:
    ...
    foreground.windowHandle` 정수 비교로 "foreground 창 본체 변경"만 통과시켜 이하
    ...
    try:
        ...
        foreground_window_handle = int(getattr(foreground, "windowHandle", 0) or 0)
    except Exception:
        log.debug("mtwn: nameChange hwnd coerce failed", exc_info=True)
        return
    if obj_hwnd == 0 or obj_hwnd != foreground_window_handle:
        return
    foreground_name = (getattr(foreground, "name", "") or "").strip()
    ...
    if not foreground_name:
        ...
        return
    appId = getAppId(obj)
    title = normalize_title(foreground_name)
    if not title:
        if debug:
            log.info(
                f"mtwn: DBG nameChange skip-normalize foreground_name={foreground_name!r}"
            )
        return
    tab_signature = obj_hwnd
    if debug:
        log.info(
...
def dispatch_focus(plugin, obj) -> None:
    """obj가 3분기 중 하나에 해당하면 raw_title/tab_signature/appId 뽑아 matcher로 위임.

    3분기:
        1) Alt+Tab 오버레이 — obj.window_class_name == ALT_TAB_OVERLAY_WCN AND foreground.window_class_name ==
           ALT_TAB_HOST_FG_WCN. obj.window_class_name(InputSite)은 UWP 공용이라 단독으로는
           Win+B 숨김 아이콘·트레이·알림 센터까지 걸린다. Xaml Shell 호스트
           foregroundClassName과 AND로 묶어 진짜 Alt+Tab/Win+Tab만 진입. 후보의 obj.appId는
           오버레이 호스트(항상 'explorer')라 무의미 → match_appId=""로 내려
           matcher의 app_lookup 조회를 스킵(title 정확 매치 + title-only 역
           매핑까지만 허용).
        2) 앱별 오버레이 (예: Notepad++ MRU) — foregroundClassName이 appId의 overlay 목록에
           있음. obj가 리스트 항목.
        3) 에디터 자식 컨트롤 — `is_editor_class(appId, window_class_name) AND window_class_name != foreground_class_name`.
           메모장 "제목 없음" 여러 탭처럼 title이 안 바뀌어 nameChange로 구분
           불가한 앱 전용. `window_class_name != foreground_class_name` 게이트가 자식==최상위 wcn인 앱
    ...
    window_class_name = getattr(obj, "windowClassName", "")
    appId = getAppId(obj)
    foreground = api.getForegroundObject()
    foreground_class_name = getattr(foreground, "windowClassName", "") if foreground is not None else ""
...
def _log_focus_diag(obj) -> None:
    ...
    window_class_name/name/role/parentClassName/foregroundClassName/foregroundName/appId 7필드로 3분기 판정 실측용.
    본 로직 침범 방지를 위해 전용 try/except로 격리.

    로그 키(parentClassName, foregroundClassName, foregroundName)는 NVDA
    속성명 컨벤션과 기존 사용자가 공유한 디버그 로그 호환성을 위해 카멜케이스로
    유지한다. 지역 변수만 snake_case로 풀네임화됐고 로그 필드명은 불변.
    """
    try:
        window_class_name = getattr(obj, "windowClassName", "") if obj is not None else ""
        ...
        parent_class_name = getattr(parent, "windowClassName", "") if parent is not None else ""
        foreground = api.getForegroundObject()
        foreground_class_name = getattr(foreground, "windowClassName", "") if foreground is not None else ""
        foreground_name = (getattr(foreground, "name", "") or "") if foreground is not None else ""
        try:
            obj_app = getAppId(obj) if obj is not None else ""
        except Exception:
            obj_app = "<err>"
        log.info(
            f"mtwn: DBG gF window_class_name={window_class_name!r} name={obj_name!r} role={role!r} "
            f"parentClassName={parent_class_name!r} foregroundClassName={foreground_class_name!r} foregroundName={foreground_name!r} "
            f"appId={obj_app!r}"
        )
    except Exception:
...
def _determine_match_source(obj, window_class_name, appId, foreground, foreground_class_name):
    ...
    in_alt_tab = (window_class_name == ALT_TAB_OVERLAY_WCN and foreground_class_name == ALT_TAB_HOST_FG_WCN)
    in_app_overlay = tabClasses.is_overlay_class(appId, foreground_class_name)
    in_tab_editor = (tabClasses.is_editor_class(appId, window_class_name) and window_class_name != foreground_class_name)

    if not (in_alt_tab or in_app_overlay or in_tab_editor):
        return None

    # 매칭 소스: alt_tab/overlay는 obj.name(후보·리스트 항목), editor는 foreground.name(활성
    # 탭 제목 포함 title bar).
    if in_tab_editor:
        src = foreground or obj
    else:
        src = obj
    ...
    # tab_signature: editor 분기는 **항상 obj(자식)**의 hwnd. foreground.windowHandle 쓰면
    # 메모장 "제목 없음" 여러 탭이 같은 최상위 hwnd를 공유해 구분 불가.
    # 나머지 분기는 title 소스와 같은 obj의 hwnd.
    signature_object = obj if in_tab_editor else src
    try:
        tab_signature = int(getattr(signature_object, "windowHandle", 0) or 0)
    except Exception:
        tab_signature = 0
