...
3. NVDA 재시작
4. (필요 시) 기존 사용자 데이터 이식: 다른 머신의 `app.json`(v9 스펙)을 `%APPDATA%\nvda\multiTaskingWindowNotifier\app.json`에 복사 후 NVDA 재시작. v9가 아닌 파일은 손상으로 취급되니 버전 확인 필수. Phase 10부터 구형 `app.list` 자동 변환은 제거됐다.
...
└── globalPlugins/
    └── multiTaskingWindowNotifier/
        ...
        ├── presets.py                     # 프리셋 데이터 단일 소유자 (PRESETS 5종, freqs 빌더, 부팅 assert, get_preset_or_classic) - Phase 7.1 신설
        ├── appIdentity.py                 # 앱 식별/복합키 + normalize_title (4단계 파이프라인: dirty → app suffix(em-dash 1순위) → count token → dangling sep)
        ├── store/                         # v9 JSON 저장소 서브패키지
        │   ├── __init__.py                # 공개 API 재export (load/save/record_switch/flush/…)
        │   ├── core.py                    # _load_state + 10개 공개 API 본체
        │   ├── io.py                      # JSON I/O + 원자적 저장 (v9 엄격 로드)
...
└── app.list.bak       # (과거 app.list 마이그레이션 이력 보존본, 수동 삭제 무해)
```
...
Phase 10 (마이그레이션 전면 제거): v1~v8 자동 승격 경로는 제거됐다. v9가 아닌
`app.json`은 "손상"으로 취급되어 빈 목록으로 시작하고 원본 파일은 덮어쓰지
않는다. 구형 `app.list`도 더 이상 자동 변환되지 않으며 남아있어도 무해.
...
- **이벤트 후킹** (3-way 책임 분리, Phase 7)
  ...
  각 분기의 raw title은 `normalize_title`을 거쳐 꼬리 " — 앱명"(em-dash 1순위) / " - 앱명" 서픽스 + 변동 `(N)` 카운트 토큰을 제거한 뒤 매칭. appId가 복합키 1등이라 title에 앱명 중복 저장하지 않는다. 브라우저 탭의 알림 카운트가 변해도 같은 키로 매칭(Phase 9.1).

  같은 키 0.3초 내 재매칭은 `_MATCH_DEDUP_SEC` 가드로 한 번만.
- **파일 저장소**
  - `store` 서브패키지(Phase 3에서 분해): 앱 목록 + 메타 JSON I/O. `store.core._states` 모듈 캐시로 상태 유지, `store.record_switch`/`store.flush`로 디바운스 저장. `store.core._load_state`는 v9 고정 로드 + `_ensure_beep_assignments`로 누락 필드만 보강하는 3단계 파이프라인 (Phase 10에서 마이그레이션 제거). I/O는 `store.io`, 비프 할당은 `store.assign`. `store.set_aliases(path, key, [str])`로 단일 entry의 alias를 즉시 원자 저장.
...
- **현재**: 4개 프리셋(classic / pentatonic / fifths / moss_bell), duration/gap_ms 조정 가능 (`config.conf`). 볼륨은 NVDA 전역 `tones` 설정 사용. Phase 11에서 hybrid 파형 합성 경로는 제거.
