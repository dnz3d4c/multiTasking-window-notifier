...
### app.json (v9, normalize_title 파이프라인 확장)
```json
{
  "version": 9,
...
- **title 정규화** (Phase B + Phase 9.1): 등록 시점(`windowInfo.get_current_window_info`)과 로드 시점(`store.core._load_state`) 모두 `normalize_title`을 거쳐 4단계 파이프라인 적용 — (1) 선두 dirty 마커 제거, (2) 꼬리 ` — 앱명`(em-dash 1순위) / ` - 앱명` 서픽스 제거, (3) `(N)` 카운트 토큰 제거(위치 무관, 1~4자리만), (4) 선두/꼬리 dangling 구분자 정리. alias도 등록/편집 시점에 동일 정규화 적용. v9에서는 디스크 저장된 aliases도 `renormalize_aliases_v9`로 1회 재정규화.
- **v8 → v9 자동 마이그레이션** (Phase 9.2): normalize_title 파이프라인 확장(em-dash 1순위 + `(N)` 카운트 토큰 흡수)에 따른 일관성 보강. title은 매 부팅 호출되는 `_normalize_titles_in_place`가 새 룰로 자동 흡수(dedup 합병 가능), aliases는 `renormalize_aliases_v9`가 1회 재정규화. 첫 마이그레이션 직전 `app.json.v8.bak` 1회 백업(이미 존재하면 보존). 비프 재배정/스키마 변경 없음.
- **v7 → v8 자동 마이그레이션**: 로드 시 모든 entry에 `aliases=[]` 주입 후 `version=8` 승격. 비프 재배정 없음 (단순 필드 확장). v9까지 같은 사이클로 연속 진행.
- **v6 → v7 자동 마이그레이션**: 반음 64음 테이블(v6)에서 C major 온음계 35음(v7)으로 교체되며 인덱스 의미 자체가 달라진다. 로드 시 기존 appBeepMap/tabBeepIdx를 전부 버리고 순차 재배정. 이어서 v7→v8→v9 자동 승격까지 한 번에 처리. 1회성, 사용자는 주파수 재학습 필요.
- **v5 이하 자동 마이그레이션**: 거리 최대화 방식(v4/v5)도 동일 경로로 한 번에 v9까지 끌어올려 재배정.
- **v3 이하 자동 마이그레이션**: appBeepMap/tabBeepIdx 필드가 부재하면 동일 재배정 경로로 v9까지 할당 + aliases 채움.
- **v2 → v3 → v9 자동 마이그레이션**: scope 필드 누락 시 `"window"`로 보정 후 v9까지 연쇄 진행.
...
- **데이터 포맷**: `app.json` v9. app.json title은 Phase B에서 꼬리 앱명 서픽스 정규화, Phase 9.1에서 em-dash 1순위 + `(N)` 카운트 토큰 흡수까지 확장(기존 데이터는 load 시 자동 마이그레이션, .v8.bak 백업 1회). 앱별 editor/overlay wcn은 `tabClasses.py` 상수. Phase 8에서 scope=window/app 양쪽 entry에 `aliases` 배열 필드 추가 — foreground title과 Alt+Tab 오버레이 이름이 다른 앱(카카오톡: "카카오톡" vs "링키지접근성") 매칭 지원.
