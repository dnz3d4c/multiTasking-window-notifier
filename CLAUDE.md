...
- **데이터 포맷**: `app.json` v9 고정 스펙. 등록 시점에 title/alias가 `normalize_title` 4단계 파이프라인을 거쳐 저장되므로 디스크 데이터는 항상 정규화된 상태. v9가 아니면 "손상"으로 취급 + 빈 목록으로 시작(원본 보존). 앱별 editor/overlay wcn은 `tabClasses.py` 상수. scope=window/app 양쪽 entry의 `aliases` 배열 — foreground title과 Alt+Tab 오버레이 이름이 다른 앱(카카오톡: "카카오톡" vs "링키지접근성") 매칭 지원.
