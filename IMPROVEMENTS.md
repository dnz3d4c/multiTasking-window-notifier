...
- 메신저 계열(SCOPE_APP+SCOPE_WINDOW) 동일 — title 변동성(오버레이 노출명 ↔ 대화창 내부 제목)으로 SCOPE_WINDOW도 깨짐.
...
- 실기 검증: Spotify SCOPE_APP / 메신저앱 SCOPE_APP+window / 메모장 SCOPE_WINDOW Ctrl+Tab 정상 비프 확인 필요.

대표 커밋: `1f3363a` (feat) + `fe50568` (chore: settings.json 권한 자동화).

### Phase 8 (완료): v8 aliases — foreground ≠ Alt+Tab 이름 앱 매칭

일부 메신저 앱처럼 foreground title("메신저앱")과 Alt+Tab 오버레이 이름("대화창제목")이 다른 앱을 단일 entry로 매칭하기 위한 aliases 필드 도입. 2026-04-19 사용자 실측 로그로 문제 재확인 후 착수.
...
**매칭 흐름 예시** (메신저앱을 scope=app으로 등록 + alias="대화창제목"):
1. Alt+Tab 오버레이 → focusDispatcher가 `match_appId="", title="대화창제목"` 전달.
2. matcher: 정확 매치 실패 → `windowLookup["대화창제목"]` 히트(scope=app entry idx) → `_meta_for`로 scope=SCOPE_APP 조회 → 단음 재생.
3. 메신저앱 foreground → foregroundWatcher가 `appId="messenger", title="메신저앱"` 전달 → appLookup 히트 → 같은 단음 재생.
...
### Phase 11 (완료) — synthEngine 전면 제거, tones.beep 단일 경로로 통합

사용자 방침 선언: "이 엔진이 거대해진건 나의 탓이다". synthEngine을 남길지 제거할지에 대해 20년+ 개발자 관점의 토론(찬/반/중재) 후 전면 제거로 결정. 근거 요지:

- **생태계 관용**: 다른 NVDA 애드온 10여종 전수 조사에서 커스텀 PCM 합성 경로를 쓰는 곳 없음. NVDA가 제공하는 `tones.beep`을 쓰는 게 관용이고 유지보수 가능성이 높다.
- **음색 축의 실제 값**: soft_retro는 classic과 주파수 동일(`freqs: BEEP_TABLE`), 변별은 파형 스펙트럼 축 하나에만 의존. 단독 재생 환경에서 triangle 배음 감쇠(1/n²)가 sine과 실측 변별된다는 증거 부재 → 음정 구조 축(classic 장조 / pentatonic 5음계 / fifths 5도 / moss_bell 단음계)으로 충분.
- **오너십**: 프로젝트 오너가 제거 의지를 명시 → 기술 판단을 넘어선 결정 가중치.

**철거**:
- 파일: `synthEngine.py` (298줄) 전체. `%TEMP%\mtwn_wavcache\*.wav` 파일 캐시는 런타임 생성 중단, OS temp cleanup에 위임.
- `beepPlayer.py`: `_play_via_synth` / `_resolve_volume` / `_play_one_beep` waveform 분기 제거. `_schedule_second_beep`은 `core.callLater(gap_ms, tones.beep, freq, duration)`로 단순화. 전체 파일 214줄 → 약 120줄.
- 프리셋: `soft_retro` (hybrid/triangle) 제거. 4종 운영(classic / pentatonic / fifths / moss_bell) — 모두 tonal.
- 설정: `CONFSPEC['beepVolume']` 제거. `_OBSOLETE_KEYS`에 추가해 기존 사용자 nvda.ini 자동 정리(1회성).
- UI: `settingsPanel`의 볼륨 슬라이더 위젯/bind/save/preview override 제거. `VOLUME_MIN/MAX` 상수 삭제. `_TYPE_CATEGORY_LABELS`에서 "hybrid" 엔트리 제거.

**사용자 저장값 마이그레이션**: `beepPreset='soft_retro'`는 `presets.get_preset_or_classic`의 미지 id 폴백으로 classic 흡수(기존 경로 재사용, 별도 마이그레이션 코드 추가 없음). 로그 1회 경고.

**스레딩**: 별도 스레딩 불필요. `tones.beep`은 비동기(NVDA 내부 전용 비프 스레드), `core.callLater`는 wx.Timer/이벤트 큐 경유라 `event_gainFocus` 핫패스 블로킹 없음. `nvwave.playWaveFile(asynchronous=True)` 제거로 오히려 Python 측 스레드 상태 관리가 줄었다.

**검증**:
- NVDA Addon Development Specialist 리뷰 통과 (반드시 수정 지적 없음).
- `uv run python build.py` 성공: `multiTaskingWindowNotifier-0.9-dev.nvda-addon` 22 files 66.0 KB.
- 테스트: 202/202 통과. `test_settings_defaults`에 `beepVolume` purge 검증 추가.
- 실기 검증 필요: NVDA 재설치 → 설정 > 창 전환 알림 ListBox 4개 확인 → 볼륨 슬라이더 미표시 확인 → 기존 저장값 `beepPreset='soft_retro'` 환경에서 classic 폴백 + 로그 경고 1회.

**상세 플랜**: `C:\Users\advck\.claude\plans\lively-sparking-frog.md` (토론 기록 포함).

---

### Phase 12 (완료): 구조 축소 리팩토링

사용자 호소 "코드 이해 어렵고 로직 복잡하다" 대응. 2020 legacy(70 LOC 단일 파일) 원형에서 출발한 프로젝트가 21 모듈 / 3,422 LOC / 66.0 KB까지 팽창한 상태를 되감는 작업. 기능 100% 보존 + pytest 202/202 무회귀 + team-debate 게이트로 각 Phase 쟁점 판정.

| 단계 | 변경 | 커밋 | 결과 |
|---|---|---|---|
| 12-1 | 개인 식별 예시 일반화("메신저앱"/"대화창제목") + `app.list` untrack | `3b29ce3` | 프라이버시 리스크 제거 |
| 12-2 | `store/` 서브패키지(4파일 767 LOC) → 단일 `store.py`(550 LOC) | `08e74ec` | 21 → 18 모듈, -5.9 KB |
| 12-3 | `foregroundWatcher` + `nameChangeWatcher` + `focusDispatcher` → `eventRouter.py` | `4439e95` | 18 → 16 모듈 |
| 12-4 | `lookupIndex.py` + `switchFlusher.py` → `matcher.py` + GlobalPlugin 흡수 | `e899133` | 16 → 14 모듈 |
| 12-5 | 축약 변수명 풀네임화 (`tab_sig` → `tab_signature`, `fg` → `foreground` 등). NVDA API 원문(hwnd/obj/appId)은 유지 | `048120a` | 사용자 지침 "연상 가능" 충족 |
| 12-6 | Phase 이력 주석 전수 제거, CLAUDE.md 중복/이력 정리, tree 블록 코드 매칭 갱신 | `036d9e1` | 불필요 주석 제거 |
| 12-7 | 교훈 문서화(`docs/refactoring-phase12-lessons.md`) + smoke test 체크리스트(`docs/smoke-test.md`) + CLAUDE.md 불변 원칙 섹션 | 본 커밋 | 재사용 가능 원칙 카탈로그 |

**누적 결과**: 21 → 14 모듈 / 3,422 → 약 2,900 LOC / 66.0 → 55.2 KB / pytest 202/202 유지. 기능 변경 0.

**운영 원칙 (team-debate 누적 추출)**: CLAUDE.md "### 불변 원칙" 섹션 5개 + `docs/refactoring-phase12-lessons.md` Part B 10개 체크리스트.

**회귀 사고**: Phase 12-2 빈 `store/` 디렉토리 잔존(Critical 즉시 처리), Phase 12-5 sed 연쇄 치환 버그(`signaturenature` 아티팩트) 즉시 정정. 둘 다 커밋 전 해결.

**상세 플랜**: `C:\Users\advck\.claude\plans\iridescent-tickling-hickey.md`.
...
- 상세 실행 계획: `C:\Users\advck\.claude\plans\reactive-snuggling-blum.md`, `C:\Users\advck\.claude\plans\groovy-tickling-reddy.md` (Phase 8 aliases), `C:\Users\advck\.claude\plans\lively-sparking-frog.md` (Phase 11 synthEngine 제거)
